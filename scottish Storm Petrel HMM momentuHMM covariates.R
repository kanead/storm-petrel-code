...
names(df)[names(df) == 'OSU.Ocean.NPP.0.083deg.8d.NPP'] <- 'NPP'
length(df$ID)
...
# remove extreme chlorophyll values
df<-head(df,-5)


# remove NAs chlorophyll values
df <- df %>%
  mutate(chloro = ifelse(is.na(chloro),0,chloro))

  df<-group_by(df, ID) %>%
  mutate(first2 = min(which(chloro == 0 | row_number() == n()))) %>%
  filter(row_number() <= first2) %>%
  select(-first2)
length(df$ID)
...
stormData <- df[,c(4,5,8,13,14,16,17)]
head(stormData)
